...
export function registerConfigure(program: Command, deps: CliDeps, emit: (d: unknown) => void): void {
  program
    .command("configure")
    ...
    .action(async (opts: { query?: string }) => {
      ...
      const configDir = g.configDir || deps.env.SCHOOLSOFT_CONFIG_DIR || defaultConfigDir(deps.home, deps.platform, deps.env);
      const existing = readConfigFile(configDir);

      let school = g.school;
      let orgId = g.orgId;
      if (!school) {
        const query = opts.query ?? (deps.prompt ? await deps.prompt("School name (e.g. Rösjöskolan): ") : undefined);
        if (!query) {
          throw new CliExit(EXIT.ERROR, "Nothing to configure: pass --school <slug> [--org-id <id>] or --query <name>.");
        }
        ...
        if (deps.prompt && hits.length > 1 && !opts.query) {
          deps.stderr(hits.map((h, i) => `${i + 1}. ${h.name} (${h.slug}, orgId ${h.orgId})`).join("\n"));
          const answer = await deps.prompt(`Pick 1-${hits.length} [1]: `);
          const n = Number(answer || "1");
          if (!Number.isInteger(n) || n < 1 || n > hits.length) throw new CliExit(EXIT.ERROR, "Invalid choice.");
          pick = hits[n - 1];
        }
        school = pick.slug;
