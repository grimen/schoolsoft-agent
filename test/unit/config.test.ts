...
import { resolveConfig, envSource, defaultConfigDir, NotConfiguredError } from "../../src/core/config.js";

const defaults = { home: "/home/u", platform: "linux" as const };

test("precedence: earlier sources win, defaults fill the rest", () => {
  const c = resolveConfig([{ school: "flags" }, { school: "env", orgId: "20" }, { callbackPort: "5000" }], defaults);
...
test("invalid user type and port are rejected", () => {
  assert.throws(() => resolveConfig([{ school: "s", userType: "alien" }], defaults), /Invalid userType/);
  assert.throws(() => resolveConfig([{ school: "s", callbackPort: "abc" }], defaults), /Invalid callbackPort/);
  assert.throws(() => resolveConfig([{ school: "s", callbackPort: 70000 }], defaults), /Invalid callbackPort/);
});

test("envSource maps SCHOOLSOFT_* and ignores empty strings", () => {
  const s = envSource({ SCHOOLSOFT_SCHOOL: "taby", SCHOOLSOFT_ORGID: "", SCHOOLSOFT_CALLBACK_PORT: "4000", OTHER: "x" });
  assert.deepEqual(s, {
    school: "taby", orgId: undefined, userType: undefined, clientId: undefined,
    callbackPort: "4000", stateDir: undefined, configDir: undefined,
  });
});

test("platform config dirs", () => {
  assert.equal(defaultConfigDir("/Users/j", "darwin"), "/Users/j/Library/Application Support/schoolsoft-agent");
  assert.equal(defaultConfigDir("/home/j", "linux", { XDG_CONFIG_HOME: "/xdg" }), "/xdg/schoolsoft-agent");
  assert.match(defaultConfigDir("C:\\Users\\j", "win32", { APPDATA: "C:\\Users\\j\\AppData\\Roaming" }), /Roaming[\\/]schoolsoft-agent$/);
});
