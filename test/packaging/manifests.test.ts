...
test("buildSkills produces one folder per host with merged metadata and an unchanged body", () => {
  ...
  for (const h of HOSTS) {
    ...
    if (existsSync(f)) { mkdirSync(join(root, "plugins", h), { recursive: true }); cpSync(f, join(root, "plugins", h, "skill-metadata.json")); }
  }
  ...
  assert.match(readFileSync(join(root, "dist/skills/hermes/schoolsoft/SKILL.md"), "utf8"), /hermes:\n\s+category: education/);
  assert.match(readFileSync(join(root, "dist/skills/openclaw/schoolsoft/SKILL.md"), "utf8"), /openclaw:\n\s+emoji: 🏫/);
});

test("the committed Claude skill copy matches the source (run make skills)", () => {
  const src = readFileSync(join(process.cwd(), "skills/schoolsoft/SKILL.md"), "utf8");
  const copy = readFileSync(join(process.cwd(), "plugins/claude/schoolsoft-skill/skills/schoolsoft/SKILL.md"), "utf8");
  assert.equal(copy, src);
  assert.equal(
    readFileSync(join(process.cwd(), "plugins/claude/schoolsoft-skill/skills/schoolsoft/references/commands.md"), "utf8"),
