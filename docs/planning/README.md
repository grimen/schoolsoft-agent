# Planning archive

Dated documents record design intent and implementation history. They can describe
past layouts or planned behavior. Use [current setup guides](../getting-started/README.md)
and [architecture](../development/architecture.md) for the current system.

## Specifications

- [Two-surface foundation](specs/2026-09-06-two-surface-foundation-design.md)
- [Parent-hosted connectors](specs/2026-09-07-parent-hosted-connectors.md)
- [Capture probe](specs/2026-09-26-capture-probe.md)

## Implementation plans

- [Two-surface foundation](plans/2026-09-06-two-surface-foundation.md)

[All documentation](../README.md)
