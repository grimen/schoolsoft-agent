...
Run `npx -y schoolsoft-agent browser install` once. For grades, documents, absence and assessment criteria also run `npx -y schoolsoft-agent login --web` once. Details in [Get started](../getting-started/README.md#4-optional-extras-only-if-you-want-them).
