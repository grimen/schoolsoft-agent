...
yet. The [support matrix](support-matrix.md) lists the existing local alternatives.
...
1. Download or clone the reviewed release on the server and open a terminal in its
   project folder. For terminal basics, see [computer basics](computer-basics.md).
