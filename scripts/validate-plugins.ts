...
export function validatePlugins(root: string): Problem[] {
  ...
  const need = (cond: unknown, file: string, message: string) => { if (!cond) problems.push({ file, message }); };
  const pkg = json(join(root, "package.json"));

  const mp = "plugins/claude/.claude-plugin/marketplace.json";
  const m = json(join(root, mp));
  need(m.name && m.owner?.name && Array.isArray(m.plugins), mp, "name, owner.name, plugins[] required");
  need(m.plugins?.map((p: any) => p.name).sort().join(",") === "schoolsoft-mcp,schoolsoft-skill", mp, "must list exactly schoolsoft-mcp and schoolsoft-skill");
  for (const p of m.plugins ?? []) {
    need(typeof p.source === "string" && existsSync(join(root, "plugins/claude", p.source)), mp, `source ${p.source} must exist`);
  ...
  need(JSON.stringify(s?.args) === JSON.stringify(["-y", "-p", `schoolsoft-agent@${pkg.version}`, "schoolsoft-agent-mcp"]), mcp, `args must pin schoolsoft-agent@${pkg.version}`);
  need(existsSync(join(root, "plugins/claude/schoolsoft-skill/skills/schoolsoft/SKILL.md")), "plugins/claude/schoolsoft-skill", "skills/schoolsoft/SKILL.md missing — run make skills");

  const mb = "plugins/mcpb/manifest.json";
  const b = json(join(root, mb));
  need(b.manifest_version && b.name && b.version === pkg.version && b.description && b.author?.name, mb, "manifest_version, name, version(=package), description, author.name required");
  need(b.server?.type === "node" && b.server?.entry_point && b.server?.mcp_config?.command, mb, "server.type/entry_point/mcp_config required");
  ...
  need(oc.mcp?.schoolsoft?.type === "local" && Array.isArray(oc.mcp.schoolsoft.command), "plugins/opencode/opencode.json", "mcp.schoolsoft local command[] required");
  ...
  need(Array.isArray(json(join(root, "plugins/pi/pi.json")).skills), "plugins/pi/pi.json", "skills[] required");
  need(JSON.stringify(pkg.pi?.skills) === JSON.stringify(json(join(root, "plugins/pi/pi.json")).skills), "package.json", "pi.skills must equal plugins/pi/pi.json");
  return problems;
}
...
export function externalValidators(root: string): { notes: string[]; problems: Problem[] } {
  ...
  const tryRun = (cmd: string, args: string[], file: string) => {
    ...
    if (r.error) { notes.push(`${cmd}: not available (skipped)`); return; }
    notes.push(`${cmd} ${args.join(" ")}: exit ${r.status}`);
    if (r.status !== 0) problems.push({ file, message: `${cmd} ${args.join(" ")} failed:\n${r.stdout}${r.stderr}` });
  };
  tryRun("claude", ["plugin", "validate", "plugins/claude"], "plugins/claude/.claude-plugin/marketplace.json");
  for (const p of ["schoolsoft-mcp", "schoolsoft-skill"]) tryRun("claude", ["plugin", "validate", `plugins/claude/${p}`], `plugins/claude/${p}`);
